...
describe('SoloSwap', () => {
  ...
  before('deploy SoloFactory and Solo pool instance', async () => {
    ...
    await soloFactory.createSoloPool(
      ...
      ONE.mul(20).div(100), // 42% - no reset
      //ONE.mul(60).div(100), // 60% - causes reset of flex pos boundaries
  ...
  describe('trader', () => {
    ...
    it('swap', async () => {
      ...
      res = await soloPool.flexPosition();
      console.log(res);

      await weth.connect(alice).approve(soloPool.address, ONE.mul(200));
      await soloPool.connect(alice).swapExactInput(0,ONE.mul(150));
      //res = await soloPool.ts_();
      //console.log(res);
      ...
      await soloPool.connect(alice).swapExactInput(0,ONE.mul(10));
      //res = await soloPool.ts_();
      //console.log(res);
      bal = await balances(alice);
      console.log(bal);
