...
describe('SoloSwap', () => {
  ...
  before('deploy SoloFactory and Solo pool instance', async () => {
    ...
    await usdc.approve(alice.address, ONE.mul(10000));
    await usdc.transfer(alice.address,ONE.mul(10000));
    let alice_usdc_balance = await weth.balanceOf(alice.address)
    console.log(alice_usdc_balance.toString());
})
  ...
  describe('trader', () => {
    ...
    it('swap', async () => {
      ...
      await usdc.connect(alice).approve(soloPool.address, ONE.mul(150));
      await soloPool.connect(alice).swapExactInput(ONE.mul(150),0);
